// Gera o .docx do CONTRATO DE COMPRA E VENDA DE EQUIPAMENTOS INDUSTRIAIS COM
// RESERVA DE DOMINIO, ja preenchido com os dados do orcamento.
//
...
/** Paragrafo justificado padrao do contrato. */
function p(text: string, opts: { bold?: boolean; center?: boolean; size?: number; indent?: number; spaceAfter?: number } = {}): Paragraph {
  return new Paragraph({
    alignment: opts.center ? AlignmentType.CENTER : AlignmentType.JUSTIFIED,
    spacing: { after: opts.spaceAfter ?? 160, line: 276 },
    indent: opts.indent ? { left: opts.indent } : undefined,
    children: [r(text, { bold: opts.bold, size: opts.size })],
  })
}

function clausula(titulo: string): Paragraph {
  return new Paragraph({
    spacing: { before: 280, after: 120 },
    children: [r(titulo, { bold: true })],
  })
}

function cell(text: string, opts: { bold?: boolean; align?: 'left' | 'center' | 'right'; shade?: string; width?: number } = {}): TableCell {
  return new TableCell({
    width: opts.width ? { size: opts.width, type: WidthType.DXA } : undefined,
    shading: opts.shade ? { type: ShadingType.CLEAR, fill: opts.shade, color: 'auto' } : undefined,
    margins: { top: 60, bottom: 60, left: 80, right: 80 },
    children: [new Paragraph({
      alignment: opts.align === 'center' ? AlignmentType.CENTER
        : opts.align === 'right' ? AlignmentType.RIGHT
          : AlignmentType.LEFT,
      spacing: { after: 0 },
      children: [r(text, { bold: opts.bold, size: 20 })],
    })],
  })
}

function assinatura(linha1: string, linha2: string): Paragraph[] {
  return [
...
function qualificacaoComprador(d: ContratoDados): string {
  ...
  const docPessoal =
    `inscrito no CPF sob o nº ${vazio(rep.cpf, 'CPF')}, portador do RG nº ${vazio(rep.rg, 'RG')}, ` +
    `nascido em ${vazio(rep.nascimento, 'DATA DE NASCIMENTO')}, filho de ${vazio(rep.mae, 'NOME DA MÃE')}`
...
function qualificacaoGarantidor(d: ContratoDados): string {
  ...
  return (
    ...
    `${vazio(g.estadoCivil, 'ESTADO CIVIL')}, ${vazio(g.profissao, 'PROFISSÃO')}, inscrito no CPF sob o nº ` +
    `${vazio(g.cpf, 'CPF')}, portador do RG nº ${vazio(g.rg, 'RG')}, nascido em ` +
    `${vazio(g.nascimento, 'DATA DE NASCIMENTO')}, filho de ${vazio(g.mae, 'NOME DA MÃE')}, residente e ` +
...
function tabelaItens(d: ContratoDados): Table {
  const linhas: TableRow[] = [
    new TableRow({
      tableHeader: true,
      children: [
        cell('Item', { bold: true, align: 'center', shade: 'D9D9D9', width: 700 }),
        cell('Descrição do equipamento / modelo / acionamentos inclusos', { bold: true, shade: 'D9D9D9', width: 5800 }),
        cell('Qtd.', { bold: true, align: 'center', shade: 'D9D9D9', width: 800 }),
        cell('Valor (R$)', { bold: true, align: 'right', shade: 'D9D9D9', width: 1700 }),
      ],
    }),
  ]
  ...
  for (const it of d.itens) {
    linhas.push(new TableRow({
      children: [
        cell(it.letra, { align: 'center', width: 700 }),
        cell(it.descricao, { width: 5800 }),
        cell(String(it.qtd), { align: 'center', width: 800 }),
        cell(it.rotulo ?? formatBRL(it.valorLinha), { align: 'right', width: 1700 }),
      ],
    }))
  }
  ...
  linhas.push(new TableRow({
    children: [
      cell('VALOR TOTAL', { bold: true, shade: 'F2F2F2', width: 700 }),
      cell('', { shade: 'F2F2F2', width: 5800 }),
      cell('', { shade: 'F2F2F2', width: 800 }),
      cell(formatBRL(d.valorTotal), { bold: true, align: 'right', shade: 'F2F2F2', width: 1700 }),
    ],
  }))
...
function tabelaParcelas(d: ContratoDados): Table {
  const linhas: TableRow[] = [
    new TableRow({
      tableHeader: true,
      children: [
        cell('Parcela', { bold: true, align: 'center', shade: 'D9D9D9', width: 1400 }),
        cell('Vencimento', { bold: true, align: 'center', shade: 'D9D9D9', width: 2600 }),
        cell('Valor (R$)', { bold: true, align: 'right', shade: 'D9D9D9', width: 2600 }),
      ],
    }),
  ]
  for (const pc of d.parcelas) {
    linhas.push(new TableRow({
      children: [
        cell(pc.numero, { align: 'center', width: 1400 }),
        cell(pc.vencimento || '—', { align: 'center', width: 2600 }),
        cell(formatBRL(pc.valor), { align: 'right', width: 2600 }),
      ],
    }))
  }
  return new Table({
...
export async function gerarContratoDocx(d: ContratoDados): Promise<Blob> {
  ...
  b.push(new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 20 },
    children: [r('CONTRATO DE COMPRA E VENDA DE EQUIPAMENTOS INDUSTRIAIS', { bold: true, size: 26 })],
  }))
  b.push(new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 320 },
    children: [r('COM RESERVA DE DOMÍNIO', { bold: true, size: 26 })],
  }))
  ...
  if (d.entrada || temParcelas) {
    ...
    if (d.entrada) {
      b.push(p(
        ...
        `vencimento em ${d.entrada.vencimento || '[DATA]'}; e`, { indent: 340 }))
    }
    if (temParcelas) {
      ...
      b.push(p(
        ...
        'mediante boletos bancários emitidos pela VENDEDORA, nos valores e vencimentos do quadro abaixo:', { indent: 340 }))
  ...
  b.push(new Paragraph({ spacing: { before: 300, after: 160 }, children: [r('TESTEMUNHAS:', { bold: true })] }))
  for (const n of ['1.', '2.']) {
  ...
  const doc = new Document({
    ...
    title: `Contrato – ${c.nome || 'cliente'} – Orçamento ${d.orcamentoNumero}`,
